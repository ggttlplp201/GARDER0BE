    ...
    <div className="explore-profile-view">
      ...
      <div className="explore-profile-header">
        ...
        <div>
          ...
          {profile.aesthetic && <div className="explore-profile-meta">{profile.aesthetic}</div>}
...
export default function ExploreModal({ open, onClose }) {
  ...
  const filtered = profiles.filter(p => {
    ...
    return (p.username || '').toLowerCase().includes(q) ||
           ...
           (p.aesthetic || '').toLowerCase().includes(q);
  });

  return (
    <div className="modal-bg open" role="dialog" aria-modal="true" aria-label="Explore" onClick={e => { if (e.target === e.currentTarget) onClose(); }}>
        ...
        {!selectedProfile && (
          <>
            <input
              ...
              placeholder="SEARCH NAME, LOCATION, AESTHETIC..."
              value={search}
              ...
              {filtered.map(p => (
                <div key={p.id} className="explore-card" onClick={() => setSelectedProfile(p)}>
                  ...
                  <div className="explore-card-info">
                    ...
                    {p.aesthetic && <div className="explore-card-meta">{p.aesthetic}</div>}
                  </div>
                </div>
              ))}
            </div>
