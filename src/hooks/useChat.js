...
export function useChat(user) {
  ...
  const otherOf = useCallback((c) => (c.user_low === user?.id ? c.user_high : c.user_low), [user]);
  const myReadOf = (c) => (c.user_low === user?.id ? c.low_last_read : c.high_last_read);
  ...
  const loadConversations = useCallback(async () => {
    ...
    const otherIds = convs.map(otherOf);
    ...
    setConversations(convs.map(c => {
      const p = pm[otherOf(c)] || null;
      return {
        id: c.id, other: p, otherId: otherOf(c),
